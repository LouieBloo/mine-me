...
import type { PartOverride } from '@mine-me/shared';
import './PartCard.css';
...
export const PartCard: React.FC<PartCardProps> = ({
  ...
  isHidden,
  isHighlighted,
  baseAssetPath = '/assets/sprites/characters/miner/',
  onReset,
  onToggleHighlight,
  onToggleVisibility,
  onChangeValue,
  ...
  const normalizedBasePath = baseAssetPath.endsWith('/') ? baseAssetPath : `${baseAssetPath}/`;
  const partImageUrl = partDef.file.startsWith('/') || partDef.file.startsWith('http')
    ? getAssetUrl(partDef.file)
    : getAssetUrl(`${normalizedBasePath}${partDef.file}`);
