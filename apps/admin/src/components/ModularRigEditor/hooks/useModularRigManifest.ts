...
export interface UseModularRigManifestOptions {
...
export function useModularRigManifest({
  target,
  initialManifestData,
  baseAssetPath,
  onSaveSuccess,
}: UseModularRigManifestOptions) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        let manifestData: SkeletonManifest | null = null;

        if (initialManifestData && initialManifestData.parts) {
          manifestData = JSON.parse(JSON.stringify(initialManifestData));
        } else if (target.type === 'character') {
          const r = await fetch(`${skeletonUrl}?t=${Date.now()}`);
          if (!r.ok) throw new Error(`Failed to load skeleton manifest (${r.statusText})`);
          manifestData = await r.json();
        } else if (target.type === 'mob') {
          if (target.mobId && target.mobId !== 'new') {
            const r = await fetchWithAuth(`/api/admin/mobs/${target.mobId}`);
            if (r.ok) {
              const mobJson = await r.json();
              if (mobJson?.animations && typeof mobJson.animations === 'object' && mobJson.animations.parts) {
                manifestData = mobJson.animations;
              }
            }
          }
          // Fallback to default skeleton manifest if mob has no custom skeleton yet
          if (!manifestData) {
            const r = await fetch(`${skeletonUrl}?t=${Date.now()}`);
            if (r.ok) {
              manifestData = await r.json();
        ...
        const itemsRes = await fetchWithAuth('/api/admin/items').catch(() => null);
        const itemsData = itemsRes && itemsRes.ok ? await itemsRes.json() : [];

        if (!active) return;

        setManifest(manifestData);
        setInitialManifest(manifestData ? JSON.parse(JSON.stringify(manifestData)) : null);
        ...
        if (!active) return;
        console.error('[useModularRigManifest] Error:', err);
        toast.error(err.message || 'Failed to load rig data');
        setLoading(false);
      }
    };
  ...
  }, [target.type, (target as any).mobId, skeletonUrl]);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      setIsSaving(true);
      let endpoint = '/api/admin/characters/skeleton';
      let method = 'PUT';
      let body: any = { manifest: currentUpdatedManifest };

      if (target.type === 'mob') {
        if (!target.mobId || target.mobId === 'new') {
          toast.error('Please save the mob base information first before saving the rig.');
          setIsSaving(false);
          return;
        }
        endpoint = `/api/admin/mobs/${target.mobId}/skeleton`;
        method = 'PUT';
        body = { manifest: currentUpdatedManifest, animations: currentUpdatedManifest };
      }
      ...
      onSaveSuccess?.(currentUpdatedManifest);
      toast.success(
        target.type === 'mob'
          ? 'Mob modular rig configuration saved successfully!'
          : 'Character sprite coordinates & dimensions saved successfully!'
      );
    } catch (err: any) {
      console.error('[useModularRigManifest] Save error:', err);
