    ...
    <div className="max-w-6xl space-y-8 relative pb-20">
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200">
        <form onSubmit={handleSave} className="p-8 space-y-6">
          <h3 className="text-xl font-black text-slate-800 border-b border-slate-100 pb-2">
            Base Stats
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="space-y-2 lg:col-span-2">
              ...
              />
              {errors.name && (
                <p className="text-red-500 text-xs font-bold mt-1 tracking-wide">
                  {errors.name}
                </p>
              )}
            </div>
            <div className="space-y-2">
              ...
              <input
                ...
                value={data.level}
                onChange={(e) => {
                  setData({ ...data, level: Number(e.target.value) });
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                value={data.health}
                onChange={(e) => {
                  setData({ ...data, health: Number(e.target.value) });
                  if (errors.health) setErrors({ ...errors, health: '' });
                }}
                className={`w-full p-3 bg-slate-50 border rounded-lg font-bold text-slate-800 transition-all ${
                  errors.health
                    ? 'border-red-500 ring-1 ring-red-500 bg-red-50'
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                value={data.defense}
                onChange={(e) => {
                  setData({ ...data, defense: Number(e.target.value) });
                  if (errors.defense) setErrors({ ...errors, defense: '' });
                }}
                className={`w-full p-3 bg-slate-50 border rounded-lg font-bold text-slate-800 transition-all ${
                  errors.defense
                    ? 'border-red-500 ring-1 ring-red-500 bg-red-50'
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                value={data.attackPercentage ?? 50}
                onChange={(e) => {
                  setData({ ...data, attackPercentage: Number(e.target.value) });
                }}
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                value={data.defendPercentage ?? 50}
                onChange={(e) => {
          ...
          <div className="flex justify-end pt-4 border-t border-slate-100">
            <button
              type="submit"
              disabled={saving}
              className="px-8 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-black rounded-xl shadow-lg transition-all cursor-pointer"
            >
      ...
      {!isNew ? (
        <div className="space-y-8">
          <ModularRigEditor
            target={{ type: 'mob', mobId: id!, mobName: data.name }}
            initialManifestData={data.animations?.parts ? data.animations : null}
            showHeader={true}
            onSaveSuccess={(updatedManifest) => {
              setData((prev: any) => ({ ...prev, animations: updatedManifest }));
            }}
          />
        </div>
      ) : (
        <div className="bg-slate-100 rounded-xl p-8 text-center border-dashed border-2 border-slate-300">
          <p className="font-bold text-slate-500">
            Save the base mob information first to configure the Modular Skeleton Rig.
          </p>
        </div>
      )}
    </div>
